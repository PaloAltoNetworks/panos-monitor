...
def generate_report_pdf(db_file, timespan):
    ...
    for fw in firewalls:
        ...
        throughput_chart = create_chart_image(chart_data['labels'], [{'label': 'Input (Mbps)', 'data': chart_data['input_data_mbps'], 'color': 'blue'}, {'label': 'Output (Mbps)', 'data': chart_data['output_data_mbps'], 'color': 'red'}], f"{title_prefix} Throughput", 'Mbps')
        ...
        chart_width = 130 
        pdf.image(throughput_chart, x=15, y=30, w=chart_width)
        pdf.image(session_chart, x=15 + chart_width + 10, y=30, w=chart_width)
        pdf.image(load_chart, x=15, y=115, w=chart_width)

    conn.close()

    # **CHANGE**: Convert the final output to a 'bytes' object.
